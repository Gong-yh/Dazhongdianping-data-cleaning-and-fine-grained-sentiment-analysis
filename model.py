...
class Model(object):
    ...
    def setup_input_placeholders(self):
        ...
        if self.hparams.mode in ['train', 'eval']:
            self.target_labels = tf.placeholder(
                tf.float32, shape=[None, self.hparams.feature_num, self.hparams.target_label_num], name='target_labels')#[*,20,4]
    ...
    def setup_embedding(self):
        ...
        # [batch,Vocab,Embed]
        #embedding : vocab_size , batch , embedding
        if self.hparams.embedding_dropout > 0 and self.is_training():
            ...
            mask = tf.cast(mask,dtype=tf.float32)#我加的
            self.embedding = mask * self.embedding
            self.embedding = tf.cast(self.embedding, dtype=tf.float32)#我加的
        ...
        # feature_embedding = tf.cast(feature_embedding, dtype=tf.float16)#我加的
    ...
    def elmo_encoder(self):
        ...
        with tf.variable_scope("elmo_encoder") as scope:
            ...
            #[Vocab,1,Embed]
            print('inputs.shape', inputs.shape())  # 9.10加
            inputs_reverse = _reverse(
                inputs, seq_lengths=self.sequence_length,
                seq_dim=0, batch_dim=1)# aixs=d 这项参数，表示沿着第d+1(d=-1表示倒数第一维)维的方向操作。
            encoder_states = []
            outputs = [tf.concat([inputs,inputs],axis=-1)]# aixs=d 这项参数，表示沿着第d+1(d=-1表示倒数第一维)维的方向操作。
            ...
            for i in range(self.hparams.num_layers):
                ...
                # output = tf.cast(output,tf.float16)#我加的
            ...
            # scalars = tf.cast(scalars,tf.float16)#我加的
            self.scalars = scalars
            weight = tf.get_variable('weight',initializer=tf.constant(0.001))
            # weight = tf.cast(weight,tf.float16)#我加的
            self.weight = weight
    ...
    def setup_attention_semantic(self):
        ...
        with tf.variable_scope("attention_semantic") as scope:
            ...
            if 'lstm' in self.hparams.rnn_cell_name.lower():
                h = tf.layers.dense(tf.concat([state.h for state in self.final_state],axis=-1),num_units, use_bias=True)#输出的维度大小num_units
            ...
            outputs = []
            state = initial_state
            for i in range(self.hparams.feature_num):#20
    ...
    def setup_clf(self):
        ...
        with tf.variable_scope("classification",reuse=tf.AUTO_REUSE) as scope:
            ...
            with tf.variable_scope("predict_clf"):
                ...
                output_layer = tf.layers.Dense(self.hparams.target_label_num) #  这里可改
                for i,state in enumerate(states):
                    semantic = hidden_layer(state)
                    logits = output_layer(semantic)#  20个  连续值

                    final_logits.append(logits)
                    # print('final_logits','==================',final_logits)

                    predict = tf.argmax(logits,axis=-1)
                    predict = tf.one_hot(predict,self.hparams.target_label_num)# -2，-1，0，1
    ...
    def train_clf_one_step(self, sess, source, lengths, targets, add_summary=False, run_info=False):
        ...
        if run_info:
            self.summary_writer.add_run_metadata(
                run_metadata, 'step%03d' % global_step)# metadata tf的元数据，是否手机关于训练的元数据，包括运行时间和内存占用数据
