...
def _reverse(input_, seq_lengths, seq_dim, batch_dim):
    ...
    # seq_axis = 0,batch_axis= 1以列为单位进行翻转，翻转的是每一行的元素
    # seq_axis = 1,batch_axis= 0以行为单位进行翻转，翻转的是每一列的元素
    # 默认都为0，以列为单位进行翻转，翻转的是每一列的元素
...
def get_config_proto(log_device_placement=True, allow_soft_placement=True,
                     num_intra_threads=0, num_inter_threads=0, per_process_gpu_memory_fraction=0.95, allow_growth=True):#per_process_gpu_memory_fraction=0.95 显存占用
    # GPU options:
...
def early_stop(values, no_decrease=3):#values是损失率的合集
    if len(values) < 2:
        return False
    best_index = np.argmin(values)#当前最好的一次的标签
